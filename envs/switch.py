...
class Switch(gym.Env):
    """
    Agents have to navigate in a small grid cell map to reach their target position which is a switch with same color
    as the agent. The map is a rectangle with a narrow corridor connecting a room on the left to the one on the right.
    The agents are initialized in a random position in a room and have to reach their target position in the other one
    traversing the corridor. The entrance of the corridor is randomized and also can only be crossed by a single agent
    per time. Agents move selecting on of the following actions ∈ {Left, Right, Up, Down, Stop} at each time step.

    Reward is assigned to the individual agent when he reach its target position. Default value is 5.
    Each agent’s observation includes:
    - agent absolute coordinate position normalized to 1. e.g. [1.0,1.0] for br corner.
    - agent target coordinate position normalized to 1.
    - agent distance to target position normalized to 1.
    - (if clock=True) the time remaining until the end of the episode normalized to 1.

    The terminating condition of this task is when all agents reach their target position or after n steps.
    """
    ...
    def __init__(self, full_observable: bool = False, random_init: bool = True, step_cost: float = 0, urgency_reward: bool = True, n_agents: int = 4, max_steps: int = 50, clock: bool = True):
        ...
        self.n_agents_dummy = n_agents+8
