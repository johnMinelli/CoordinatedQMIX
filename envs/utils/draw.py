...
def draw_grid(cols, rows, cell_size=50, fill='black', line_color='black'):
...
def fill_cell(image, pos, cell_size=None, fill='black', margin=0):
    ...
    cell_size_x, cell_size_y = get_cell_sizes(cell_size)
    col, row = pos
    col, row = col * cell_size_x, row * cell_size_y
    margin_x, margin_y = margin * cell_size_x, margin * cell_size_y
    x, y, x_dash, y_dash = col + margin_x, row + margin_y, col + cell_size_x - margin_x, row + cell_size_y - margin_y
...
def draw_cell_outline(image, pos, cell_size=50, fill='black'):
    ...
    col, row = pos
    col, row = col * cell_size_x, row * cell_size_y
    ImageDraw.Draw(image).rectangle([(col, row), (col + cell_size_x, row + cell_size_y)], outline=fill, width=3)
...
def draw_circle(image, pos, cell_size=50, fill='black', radius=0.3):
    ...
    x, y = col + gap_x, row + gap_y
    x_dash, y_dash = col + cell_size_x - gap_x, row + cell_size_y - gap_y
