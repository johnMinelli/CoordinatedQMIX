...
class Transport(gym.Env):
    ...
    def __init__(self, grid_size=(16, 16), n_agents=2, n_loads=4, full_observable=False, step_cost=-0.01, dock_reward=5,
        ...
        self._obs_high = np.array([1., 1.] + [1., 1.] + [1.] * (self.n_loads * 2), dtype=np.float32)
        self._obs_low = np.array([0., 0.] + [0., 0.] + [0.] * (self.n_loads * 2), dtype=np.float32)
        ...
        self._init_agent_pos.append([self._init_load_pos[-1][0] - 1, self._init_load_pos[-1][1] - 1])
        self._init_agent_pos.append([self._init_load_pos[-1][0] - 1, self._init_load_pos[-1][1] + 1])
        if self.n_loads >= 2:
            self._grid_shape = [(grid_size[0] * 2) - grid_size[1], grid_size[1]]
            ...
            self._init_agent_pos.append([self._init_load_pos[-1][0] + 1, self._init_load_pos[-1][1] + 1])
        if self.n_loads >= 3:
            self._grid_shape = [(grid_size[0] * 2) - grid_size[1], grid_size[0]]
            self._init_load_pos.append([self._grid_size[0] - math.ceil(self._grid_size[1] / 2), 1])
            self._init_agent_pos.append([self._init_load_pos[-1][0] - 1, self._init_load_pos[-1][1] - 1])
            self._init_agent_pos.append([self._init_load_pos[-1][0] + 1, self._init_load_pos[-1][1] - 1])
        if self.n_loads >= 4:
            self._grid_shape = [(grid_size[0] * 2) - grid_size[1], (grid_size[0] * 2) - grid_size[1]]
            self._init_load_pos.append(
    ...
    def __create_grid(self):
        ...
        # add walls
        for i in range(_grid.shape[1]):
            for j in range(_grid.shape[2]):
                if self._grid_shape[1] > self._grid_size[1] and (
                    i < self._grid_size[0] - self._grid_size[1] or i >= self._grid_size[0]) and (
                    j < self._grid_size[0] - self._grid_size[1] or j >= self._grid_size[0]):
                    _grid[WALL][i][j] = 1
        return _grid
    ...
    def __init_world(self, test=False):
        ...
        for i in range(self.n_agents):
            if test:
                self.agent_pos[i] = self._dock_pos
            else:
                low_end_x, high_end_x = min(self._init_agent_pos[i][0], self._dock_pos[0]), max(self._init_agent_pos[i][0], self._dock_pos[0])+1
                low_end_y, high_end_y = min(self._init_agent_pos[i][0], self._dock_pos[0]), max(self._init_agent_pos[i][0], self._dock_pos[0])+1
                while 1:
                    pos = [np.random.randint(low_end_x, high_end_x), np.random.randint(low_end_y, high_end_y)]
                    if self.is_valid(pos) and not self._wall_exists(pos) and self._is_cell_loadless(pos):
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            _agent_i_obs = [1-round(pos[0] / (self._grid_shape[0] - 1), 2),
                            1-round(pos[1] / (self._grid_shape[1] - 1), 2)]  # coordinates
            norm_x = lambda x: round(x / (self._grid_shape[0] - 1), 2)
            norm_y = lambda y: round(y / (self._grid_shape[1] - 1), 2)

            for l_pos in self.load_pos.values():
                if l_pos != self._dock_pos:
                    _agent_i_obs.append(norm_x(abs(pos[0] - l_pos[0])))
                    _agent_i_obs.append(norm_y(abs(pos[1] - l_pos[1])))
                else:
                    _agent_i_obs += [0., 0.]

            _agent_i_obs.append(norm_x(abs(pos[0] - self._dock_pos[0])))
            _agent_i_obs.append(norm_y(abs(pos[1] - self._dock_pos[1])))
            #
            # # check if prey is in the view area
            # x_m = math.floor(self._agent_view_range[0] / 2)
            # y_m = math.floor(self._agent_view_range[1] / 2)
            # _human_pos = np.zeros(self._agent_view_range)  # human location in neighbour
            ...
            #             continue
            #         if self._full_obs[LOAD][col][row] > 0:
            #             _human_pos[i, j] = min(self._full_obs[LOAD][col][row], 1)  # get relative position for the hum
            #         if self._full_obs[AGENT][col][row] > (1 if pos == [col, row] else 0):
            #             _agent_pos[i, j] = min(self._full_obs[AGENT][col][row], 1)
            #
            # _agent_i_obs += _obstacles_pos.flatten().tolist()  # adding walls pos in observable area
            # _agent_i_obs += _human_pos.flatten().tolist()  # adding human pos in observable area
            # _agent_i_obs += _agent_pos.flatten().tolist()  # adding agents pos in observable area
    ...
    def _is_cell_loadless(self, pos):
        return self.is_valid(pos) and (self._full_obs[LOAD][pos[0]][pos[1]] == 0)
    ...
    def __update_agent_pos(self, agent_i, move):
        ...
        if next_pos != curr_pos and self.is_valid(next_pos) and not self._wall_exists(
            next_pos) and self._is_cell_loadless(next_pos):
    ...
    def step(self, agents_action):
        ...
        def check_movement(agents_list_1, agents_list_2, current_load_i):
            for agent_1 in agents_list_1 + agents_list_2:
                rewards[agent_1] += 0.5
            if agents_list_1 and agents_list_2:
                for agent_1 in agents_list_1:
                    # if agents_action[agent_1] != 4: rewards[agent_1] += 0.50
                    for agent_2 in agents_list_2:
                        # if agents_action[agent_2] != 4: rewards[agent_2] += 0.50
                        if agents_action[agent_1] == agents_action[agent_2]:
                            agent_1_pos = copy.copy(self.agent_pos[agent_1])
                            next_agent_1_pos = self.__next_pos(agent_1_pos, agents_action[agent_1])
                            agent_2_pos = copy.copy(self.agent_pos[agent_2])
                            next_agent_2_pos = self.__next_pos(agent_2_pos, agents_action[agent_2])
                            cur_load_pos = copy.copy(self.load_pos[current_load_i])
                            next_load_pos = self.__next_pos(cur_load_pos, agents_action[agent_2])

                            if (agent_1_pos != next_agent_1_pos and self.is_valid(
                                next_agent_1_pos) and not self._wall_exists(next_agent_1_pos) and (
                                self._is_cell_loadless(
                                    next_agent_1_pos) or next_agent_1_pos == cur_load_pos) and agent_2_pos != next_agent_2_pos and self.is_valid(
                                next_agent_2_pos) and not self._wall_exists(next_agent_2_pos) and (
                                self._is_cell_loadless(
                                    next_agent_2_pos) or next_agent_2_pos == cur_load_pos) and self._is_cell_loadless(
                                next_load_pos) and not self._wall_exists(next_load_pos)):
                                # Move and reward
                                ...
                                self.load_pos[current_load_i] = next_load_pos
                                ...
                                if next_load_pos == self._dock_pos:
                                    ...
                                    self._update_docking_distance(current_load_i)
                                ...
                                # Remove from action dict
                                del agents_action[agent_1]
                                del agents_action[agent_2]
                                return True

        # Move humans if valid agents are present
        for load_i in range(self.n_loads):
            if self.load_pos[load_i] != self._dock_pos:
                load_pos = self.load_pos[load_i]

                # Collect agents on the left, right, top, and bottom of the current weight load
                agents_on_left = []
                agents_on_right = []
                agents_on_top = []
                agents_on_bottom = []
                ...
                for agent_i in range(self.n_agents):
                    if not (self._agent_dones[agent_i]) and agent_i in agents_action:
                        agent_pos = self.agent_pos[agent_i]
                        # Check if agent is on the left of the human
                        if agent_pos[0] == load_pos[0] - 1 and agent_pos[1] == load_pos[1]:
                            agents_on_left.append(agent_i)
                        # Check if agent is on the right of the human
                        elif agent_pos[0] == load_pos[0] + 1 and agent_pos[1] == load_pos[1]:
                            agents_on_right.append(agent_i)
                        # Check if agent is on the top of the human
                        elif agent_pos[0] == load_pos[0] and agent_pos[1] == load_pos[1] - 1:
                            agents_on_top.append(agent_i)
                        # Check if agent is on the bottom of the human
                        elif agent_pos[0] == load_pos[0] and agent_pos[1] == load_pos[1] + 1:
                            agents_on_bottom.append(agent_i)

                # Check if the human can be moved by a pair of agents on the top and bottom or on the left and right
                check_movement(agents_on_top, agents_on_bottom, load_i) or check_movement(agents_on_left,
                                                                                          agents_on_right, load_i)
...
ACTION_MEANING = {0: "RIGHT", 1: "UP", 2: "LEFT", 3: "DOWN", 4: "NOOP", }
