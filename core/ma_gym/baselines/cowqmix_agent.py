...
# from modules.mixers.qmix_central_no_hyper import QMixerCentralFF  # TO DO


"""Gym agent not fully implemented"""
class CoordQMixGymAgent(BaseAgent):
    ...
    def __init__(self, opt, env: Env, device: torch.device):
        ...
        self.central_mix = QMixerCentralFF(env.observation_space, hidden_size=self.mixer_params["hidden_size"])  # Feedforward network that takes state and agent utils as input
        self.central_mix_target = QMixerCentralFF(env.observation_space, hidden_size=self.mixer_params["hidden_size"])
