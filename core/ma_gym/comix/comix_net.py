...
class QPolicy(nn.Module):
    ...
    def __init__(self, agents_ids, observation_space, action_space, model_params=None):
        ...
        self.ma_coordinator = Coordinator(self.agents_ids, self.num_agents_dummy, self.action_space, plan_size=self.plan_size, coord_recurrent_size=self.coord_recurrent_size)
    ...
    def forward(self, state, rnn_hxs, glob_rnn_hxs, dones_mask, comm_plans=None, coord_masks=None, rec_intents=None, comm_ts_delays=None, train_coord=False, train_ae=False):
        ...
        qs, inv_qs = self._coordinate_intentions(solo_qs, rnn_hxs, scaled_intents, blind_coord_masks[:,:self.num_agents], dones_mask[:,:self.num_agents], train_coord)
