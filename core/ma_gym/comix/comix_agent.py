...
class Policy(nn.Module):
    ...
    def __init__(self, agents_ids, observation_space, action_space, base_kwargs=None):
        ...
        if self.shared_comm_ae:
            ...
            self.ae = EncoderDecoder(self.hidden_size+list(self.action_space.values())[0].n, self.comm_size)  # this broke something
        else:
            self.comm_size = self.obs_size
            self.comm_processor = _to_ma(nn.Sequential(
                nn.Linear(self.comm_size, self.hidden_size * 4), nn.ReLU(),
                nn.Linear(self.hidden_size * 4, self.hidden_size * 2), nn.ReLU(),
    ...
    def init_comm_msgs(self, batch_size=1):
        if self.shared_comm_ae:
            ...
            return torch.zeros((batch_size, self.num_agents, self.obs_size+list(self.action_space.values())[0].n)).to(self._device)
    ...
    def forward(self, input, rnn_hxs, glob_rnn_hxs, comm, dones, eval_coord=False):
        ...
        if self.shared_comm_ae:
            ...
            # alternatively pass the entire observation
            current_plans = torch.cat([input, solo_actions], dim=1)
        ...
        if self.shared_comm_ae:
            ...
            # shared input processor to reduce the complexity in input
            for i in range(self.num_agents):
                proc_comm = self.fc_processor(comm[i])
            proc_comm = torch.cat(proc_comm, dim=0) * dones

        # modify your prediction introducing coordination
        qs, inv_q, glob_rnn_hxs, coordinators_mask = self.ma_coordinator(qs, current_plans, proc_comm, rnn_hxs, glob_rnn_hxs, dones, eval_coord)  # TODO rnn_hxs.detach() maybe or maybe not
...
class CoordQMixGym(object):
    ...
    def __init__(self, opt, env: Env, device: torch.device):
        ...
        self.model_params = {"hidden_size": 64, "coord_recurrent_size": 256, "comm_size": 10}
        ...
        if opt.isTrain:
            ...
            if not self.q_policy.shared_comm_ae:
                policy_net_params += self.q_policy.comm_processor.parameters()
            ...
            self.optimizer_policy = optim.Adam(policy_net_params, lr=opt.lr)
            self.optimizer_coordinator = optim.Adam(coordinator_net_params, lr=opt.lr)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        losses = {"ae_loss": self.ae_loss.item()}
        self.ae_loss = 0
        ...
        if self.memory.size() > self.warm_up_steps:
            ...
            # TODO io vorrei clippare i gradient sulla prima action selection quando avviene la coordinazione
