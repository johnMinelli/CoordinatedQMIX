...
class CoordQMixGymAgent(BaseAgent):
    ...
    def __init__(self, opt: Namespace, env: Env, device: torch.device):
        ...
        self.q_params = {"num_dummy_agents": env.n_agents_dummy, "eval_coord_mask": opt.coord_mask_type, "ae_comm": bool(opt.ae_comm), "hidden_size": opt.hi, "coord_recurrent_size": opt.hc, "ae_comm_size": 16, "input_proc_size": opt.hs, "cnn_input_proc": bool(opt.cnn_input_proc)}
        ...
        if self.training or self.fine_tuning:
            ...
            self.tau = opt.tau
            self.q_threshold = opt.q_threshold
            ...
            if coord_training:
                ...
                if self.q_policy.eval_coord_mask == "optout":
                    ...
                    loss = torch.sum(torch.sum(
                        nn.ReLU()(inv_pred_q_s - pred_q_s.unsqueeze(1).expand((max_inv_q_a.size(0), self.q_policy.num_agents_dummy, self.q_policy.num_agents))).unsqueeze(-1).expand(coord_masks.shape) * coord_masks, -1))
                else:
