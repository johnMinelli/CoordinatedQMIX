...
class Coordinator(nn.Module):
    ...
    def __init__(self, agents_ids, num_dummy_agents, action_space, plan_size, coord_recurrent_size):
    ...
    def forward(self, plans, comm_plans, glob_hiddens):
        ...
        glob_batch_mask = torch.cat([glob_batch_mask, torch.ones((self.num_agents_dummy-glob_batch_mask.size(0), *glob_batch_mask.shape[1:]), device=glob_batch_mask.device)]).bool()
        for i in range(self.num_agents):
            ...
            x = torch.cat([x, torch.randn((self.num_agents_dummy-x.size(0), *x.shape[1:]), device=glob_batch_mask.device)])
